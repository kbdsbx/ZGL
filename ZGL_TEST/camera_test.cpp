...
namespace ZGL_TEST
{
	...
	public:
		...
		TEST_METHOD(camera_rotate)
		{
			...
			// 翻滚
			cm.roll(1.5);
			Assert::IsTrue(cm.forward() == ZGL::affine_vector< 4, double > { -0.93704257133163638, 0.31234752377721214, 0.15617376188860607, 0 });
			Assert::IsTrue(cm.rightward() == ZGL::affine_vector< 4, double > { -0.34073749202922399, -0.91571209082311256, -0.21300077052911853, 0 });
			Assert::IsTrue(cm.upward() == ZGL::affine_vector< 4, double > { -0.076479938793318034, 0.25280504565891793, -0.96448972407774403, 0 });

			// Camera pitch
			// 俯仰
			cm.pitch(1.5);
			Assert::IsTrue(cm.forward() == ZGL::affine_vector< 4, double > { -0.14257212486162851, 0.27426635541281591, -0.95102636950875941, 0 });
			Assert::IsTrue(cm.rightward() == ZGL::affine_vector< 4, double > { -0.34073749202922299, -0.91571209082311289, -0.21300077052911873, 0 });
			Assert::IsTrue(cm.upward() == ZGL::affine_vector< 4, double > { 0.92928529028392315, -0.29368236754857108, -0.22400784864149409, 0 });

			// Camera yaw
			// 偏航
			cm.yaw(1.5);
			Assert::IsTrue(cm.forward() == ZGL::affine_vector< 4, double > { 0.32979878689865894, 0.93281905426227385, 0.14519425665435209, 0 });
			Assert::IsTrue(cm.rightward() == ZGL::affine_vector< 4, double > { -0.16631779646838013, 0.20880440368034114, -0.96371111417353672, 0 });
			Assert::IsTrue(cm.upward() == ZGL::affine_vector< 4, double > { 0.92928529028392326, -0.29368236754857119, -0.22400784864149414, 0 });
		}

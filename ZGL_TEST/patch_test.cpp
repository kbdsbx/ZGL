...
namespace ZGL_TEST
{
	...
	public:
		...
		TEST_METHOD(patch_init)
		{
			...
			Assert::IsTrue(p.pos == ZGL::affine_vector< 3, double >{ -5, 3, .5, 1 });
			...
			Assert::IsTrue(p.vertexes[1] == ZGL::affine_vector< 3, double > { 6, 0, -3.5, 1 });
		}

		TEST_METHOD(patch_n)
		{
			ZGL::patch< 2, 1, double > p({
				{ -5, 3, 1 },
				{ 6, 0, 1 },
			});
			...
			Assert::IsTrue(p.n() == ZGL::affine_vector< 2, double > { -0.26311740579210879, -0.96476382123773219, 0 });
		}
	};
}